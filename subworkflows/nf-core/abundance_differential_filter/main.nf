...
workflow ABUNDANCE_DIFFERENTIAL_FILTER {
    ...
    def criteria = multiMapCriteria { meta_input, abundance, analysis_method, fc_threshold, stat_threshold, meta_exp, samplesheet, meta_contrasts, variable, reference, target, formula, comparison ->
        ...
        samples_and_matrix: [meta_input_new, samplesheet, abundance]
        contrasts_for_diff: [meta_for_diff, variable, reference, target]
        contrasts_for_diff_with_formula: [meta_for_diff, variable, reference, target, formula, comparison]
        filter_params: [meta_for_diff, ['fc_threshold': fc_threshold, 'stat_threshold': stat_threshold]]
        contrasts_for_norm: [meta_input_new, variable, reference, target]
    }
    ...
    dream_inputs = inputs.contrasts_for_diff_with_formula.filter { meta, _variable, _reference, _target, formula, _comparison ->
        ...
        .multiMap { meta, results, filter_meta ->
            def method_params = [
                ...
                'propd': [
                    fc_column: 'lfc',
                    fc_cardinality: '>=',
                    stat_column: 'weighted_connectivity',
                    stat_cardinality: '>=',
                ],
                'dream': [
                    fc_column: 'logFC',
