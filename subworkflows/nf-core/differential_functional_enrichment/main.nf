...
workflow DIFFERENTIAL_FUNCTIONAL_ENRICHMENT {
    ...
    ch_input = ch_input.multiMap { meta_input, file, genesets, background, analysis_method ->
        def meta_new = meta_input + ['method_functional': analysis_method]
        input: [meta_new, file]
        genesets: [meta_new, genesets]
        background: [meta_new, background]
    }
    ...
    def criteria = multiMapCriteria { meta_input, input, genesets, meta_exp, samplesheet, featuresheet, features_id, features_symbol, meta_contrasts, variable, reference, target, _formula, _comparison ->
        def meta_contrasts_new = meta_contrasts + ['variable': variable, 'reference': reference, 'target': target]
        // make sure variable, reference, target are in the meta
        def meta_all = mergeMaps(meta_contrasts_new, meta_input)
        input: [meta_all, input]
        genesets: [meta_all, genesets]
        contrasts_and_samples: [meta_all, samplesheet]
        features: [meta_exp, featuresheet]
        features_cols: [features_id, features_symbol]
    }
