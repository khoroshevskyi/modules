...
workflow VCF_IMPUTE_GLIMPSE {
    ...
    input_chunk = ch_input.map { meta, vcf, csi, sample, region, ref, ref_index, map ->
        [meta, vcf, csi, region]
    }
    ...
    phase_input = ch_input
        .map { meta, vcf, csi, sample, region, ref, ref_index, map -> [meta, vcf, csi, sample, ref, ref_index, map] }
    ...
    // [meta, vcf, index, sample_infos, regionin, regionout, ref, ref_index, map]
    ...
    ligate_input = GLIMPSE_PHASE.out.phased_variants
        .groupTuple(by: 0)
        .combine(
            INDEX_PHASE.out.csi.groupTuple(by: 0),
            by: 0
        )
